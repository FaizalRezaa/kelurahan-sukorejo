...
import { Edit2, Search, Check, ExternalLink, Layout } from "lucide-react";
...
// ─────────────────────────────────────────────────────────────────────────────
// Section A — 6 Kotak Layanan Publik (resource grid)
// ─────────────────────────────────────────────────────────────────────────────

function LayananPublikSection() {
  const [items, setItems] = useState<LayananPublikRecord[]>(layananPublikData);
  const [search, setSearch] = useState("");
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<LayananPublikRecord | null>(null);
  ...
  const handleOpenEdit = (item: LayananPublikRecord) => {
    setEditingItem(item);
      ...
      <Card className="p-0 overflow-hidden">
        ...
        <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50">
          <p className="text-[11px] text-slate-400">
            6 baris tetap · Tidak dapat ditambah atau dihapus · Hanya bisa diedit
          </p>
        </div>
      </Card>
          ...
          <form onSubmit={handleSave} className="space-y-3 pt-2">
            <div>
              ...
              <Input
                type="text"
                required
                placeholder="Contoh: Layanan Surat & Administrasi"
                value={formJudul}
            ...
            <div>
              ...
              <Input
                type="text"
                required
                placeholder="Contoh: /layanan atau https://..."
                value={formHref}
                onChange={(e) => setFormHref(e.target.value)}
              />
              <p className="mt-1 text-[11px] text-slate-400">
                Path relatif (contoh: /layanan) atau URL absolut.
              </p>
            </div>
            ...
            <div>
              ...
              <Textarea
                rows={3}
                required
                placeholder="Deskripsi singkat yang tampil saat hover..."
                value={formDeskripsi}
...
function BannerSection() {
  const [items, setItems] = useState<BannerRecord[]>(bannerData);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<BannerRecord | null>(null);
  ...
  const handleOpenEdit = (item: BannerRecord) => {
    setEditingItem(item);
    setFormJudul(item.judul);
      ...
      <Card className="p-0 overflow-hidden">
        <CardHeader className="px-6 pt-6 pb-4">
          <div className="flex items-center gap-2 mb-1">
            <Layout className="h-4 w-4 text-slate-500" />
            <CardTitle className="text-base font-semibold text-slate-900">
              2 Banner Call-to-Action Beranda
            </CardTitle>
          </div>
          <CardDescription>
                    ...
                    <TableCell className="max-w-[220px]">
                      <div className="flex items-center gap-3">
                        <img
                          src={item.image_path}
                          alt={item.judul}
                          className="h-10 w-16 rounded-md object-cover border border-slate-200 shrink-0"
                        />
        ...
        <div className="px-6 py-3 border-t border-slate-100 bg-slate-50/50">
          <p className="text-[11px] text-slate-400">
            2 baris tetap · Tidak dapat ditambah atau dihapus · Hanya bisa diedit
          </p>
        </div>
      </Card>
          ...
          <form onSubmit={handleSave} className="space-y-3 pt-2">
            <div>
              ...
              <Input
                type="text"
                required
                placeholder="Contoh: Cari Produk Hukum?"
                value={formJudul}
                onChange={(e) => setFormJudul(e.target.value)}
              />
            </div>
            ...
            <div>
              ...
              <Input
                type="text"
                required
                placeholder="Contoh: Lihat Produk Hukum"
            ...
            <div>
              ...
              <Input
                type="text"
                required
                placeholder="Contoh: /produk-hukum atau https://..."
                value={formHref}
                onChange={(e) => setFormHref(e.target.value)}
              />
            </div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-lg border border-amber-100 bg-amber-50 px-4 py-3 text-xs text-amber-700 flex items-start gap-2">
        ...
        <span>
          Perubahan yang disimpan di sini akan langsung tercermin di halaman beranda publik setelah
          integrasi dengan backend selesai. Pastikan gambar, teks, dan link sudah benar sebelum
          dipublikasikan.
        </span>
      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-bold uppercase tracking-wide text-slate-600">
            Banner Call-to-Action (2 Bagian)
          </h2>
        </div>
        <BannerSection />
      </section>
